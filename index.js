  ...
  conn.ev.on("messages.upsert", async ({ messages }) => {
    try {
      ...
      const groupAdmins = isGroup
        ? _.map(_.filter(groupMembers, "admin"), "id")
        : "";
      const q = args.join(" ");
      ...
      switch (comando) {
        ...
        case "dono":
          ...
          try {
            conn.sendMessage(sender, {
              contacts: { displayName: `${nomeDono}`, contacts: [{ vcard }] },
            });
          } catch (e) {
            console.log(e);
          }
        ...
        case "hidetag":
          if (!isGroup)
            return enviar("Este comando só poderia ser utilizado em grupo.");
          if (!isGroupAdmins)
            return enviar("Somente admins poderia utilizar esse comando.");
        ...
        case "cardapio_add":
          //cria um cardapio com todos os produtos disponiveis para venda
          
          break;

        case "cardapio_delete":
          //deleta itens do cardapio
          break;

        case "cardapio":
        //  lista todos os produtos a venda
